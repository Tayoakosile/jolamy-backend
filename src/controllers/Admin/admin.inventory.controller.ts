...
export const getAllInventory = async (req: Request, res: Response) => {
  try {
    const products = (await Product.find().populate("orders")) as IProduct[];
    ...
    const all_distributors_stock = distributors.map((distributor) => {
      const currentStockLog =
        distributor?.stock_logs?.[distributor.stock_logs.length - 1];
      if (!currentStockLog) return;
      return {
        ...
        status:
          (currentStockLog?.new_stock ?? 0) < 50
            ? "critical"
